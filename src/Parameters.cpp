...
//----General options-----------------------------------------
	...
	//set solar position
	par_list.push_back(ParMem("rSun[0]",&posC[0],DOUBLE,"-8.0",1));
	par_list.push_back(ParMem("rSun[1]",&posC[1],DOUBLE,"0.0",1));
	par_list.push_back(ParMem("rSun[2]",&posC[2],DOUBLE,"0.015",1));
	par_list.push_back(ParMem("vSun[0]",&posC[3],DOUBLE,"11.1",1));
	par_list.push_back(ParMem("vSun[1]",&posC[4],DOUBLE,"239.08",1));  //226.84+12.24
	par_list.push_back(ParMem("vSun[2]",&posC[5],DOUBLE,"7.25",1));
...
//		}
	...
	{
		outputFile+=".";
		outputFile+=nbodyFile;  //append name of simulation to output file name 
		popID=10;
		load_sat_list();
...
void Parameters::load_sat_list()
{
	...
	if((fd=fopen(s.c_str(),"r")))
	{
		...
		sscanf(buf,"%d",&sats);
		sscanf(buf,"%d",&x[0]);
		cerr<<"path "<<path<<" sats "<<sats<<" x[0] "<<x[0]<<endl;
//		for(int i=0;i<sats;++i)
		...
		while(1)
		{
			...
			cout<<"Halo "<<x[0]<<" Sat "<<x[1]<<" "<<"fname "<<s1<<" "<<feof(fd)<<endl;
		}
		if(int(sat_list.size())!=sats)
		{
			cerr<<"sats!=sat_list.size() "<<sats<<"!="<<sat_list.size()<<" check "<<" file"<<endl;
			exit(1);
		}
		fclose(fd);
